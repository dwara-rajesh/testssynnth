...
// Tunables: ±2‑octave transpose, more sine bias, LPF, attack envelope, tighter reverb
...
static constexpr unsigned int CHAN     = 2;                          // stereo
...
static constexpr double LPF_ALPHA = 0.1;          // one-pole filter α
static constexpr double ATTACK_MS = 10.0;         // 10 ms attack
static constexpr int    ENV_SAMPLES = int(FS*(ATTACK_MS/1000.0));
...
struct Osc {
    ...
    double sine()    { double v=std::sin(2*M_PI*phase); phase+=step; if(phase>=1.0) phase-=floor(phase); return v; }
...
void visionThread(){
    ...
    while(running){
        ...
        if(ENABLE_FEEDBACK_LOG) std::cout<<"[WARMTH_DBG] rawWarmth="<<rawWarmth<<"\n";
        ...
        Mat thr, clean;
        adaptiveThreshold(gray,thr,255,ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY_INV,11,2);
        morphologyEx(thr,clean,MORPH_OPEN,getStructuringElement(MORPH_RECT,Size(3,3)),Point(-1,-1),2);
...
void audioThread(){
    ...
    unsigned int rate=FS; snd_pcm_hw_params_set_rate_near(pcm,hw,&rate,nullptr);
    snd_pcm_uframes_t per=PERIOD; int dir=0;
    snd_pcm_hw_params_set_period_size_near(pcm,hw,&per,&dir);
    snd_pcm_hw_params(pcm,hw); snd_pcm_hw_params_free(hw);
    snd_pcm_prepare(pcm);
    ...
    int    delayPos    = 0;
    double lpfState    = 0.0;
    double envelopeGain= 1.0, envelopeInc = 1.0;
    ...
    while(running){
        ...
        if(frameCount >= nextBarFrame){
            ...
            else             std::rotate(seq.begin(),seq.begin()+2,seq.end());
            ...
            { // commit & trigger attack
                std::lock_guard<std::mutex> lk(seqMutex);
                globalSeq = std::move(seq);
                osc.init(freqLUT[ globalSeq.front() ]);
            }
            envelopeGain = 0.0;
            envelopeInc  = 1.0/ENV_SAMPLES;
            subCount     = 0;
        }
        ...
        float revMix = float(remap(sens,0.0,1.0,0.3,0.5));
        ...
        auto wave = [&](){
            if(tx<0.07f)        return osc.sine();
            else if(tx<0.14f)   return osc.triangle();
        ...
        for(unsigned i=0;i<PERIOD;++i,++frameCount,++subCount){
            ...
            // LPF
            lpfState += LPF_ALPHA*(dry - lpfState);
            dry = lpfState;

            // attack envelope
            envelopeGain = std::min(envelopeGain + envelopeInc, 1.0);
            dry *= envelopeGain;
